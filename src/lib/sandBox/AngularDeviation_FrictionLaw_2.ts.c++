...
export class AngularDeviation_FrictionLaw_2 /*extends MisfitCriteriun*/ {
    ...
    value(stress: Matrix3x3): number {
        ...
        frictionStriaUnitVecLocal[2] = Math.cos( this.frictionAngleRock_ )
            ...
            if ( magStressShiftSigma_n > epsilon) {    // stressMag > Epsilon ***
                ...
                Flocal[0] = scalarProductUnitVectors({U: F, V: fault.striation})
                ...
                Flocal[1] = Math.abs( scalarProductUnitVectors({U: F, V: fault.e_perp_striation}) )
